  ...
  return (
    <div className="flex items-center gap-3 p-4">
      ...
      <span className="min-w-0 flex-1">
        <span className="block text-[17px] font-semibold">{t.perfil.sonido.titulo}</span>
        <span className="block text-[13px] text-muted">{t.perfil.sonido.descripcion}</span>
      </span>
      <button
        type="button"
        role="switch"
        aria-checked={activo}
        aria-label={t.perfil.sonido.titulo}
        onClick={alternar}
